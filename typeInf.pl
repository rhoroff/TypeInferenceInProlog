...
typeStatement(ifStatement(Cond, Code), T):-
    typeExp(Cond, bool),
    typeExp(Code,T).

typeStatement(ifElseStatement(Cond1,T1,T2), T):- 
    typeExp(Cond1,bool),
    typeExp(Code1,T2),
    typeExp(Code2,T2),
    T1 == T2,
...
fType(printStatement, [_X, unit]). /* simple print */
fType(lessThan, [int, int, bool]).
fType(greaterThan, [int, int, bool]).
fType(equal, [int,int,bool]).
fType(notEqual,[int,int,bool]).
